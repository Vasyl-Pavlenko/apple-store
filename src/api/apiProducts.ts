...
export const BASE_URL = 'https://github.com/Vasyl-Pavlenko/apple-store.git';

export const FAKE_BASE_URL = '.';
export const LOCAL_IMG_URL = '/apple-store';
...
export function homePageLoader() {
  const newProducts = removeUnavailable()
    .then(products => products.filter(({ year }) => year === 2023))
    .then(products => {
      const copy = [...products];
